		...
		pickerchange: function(picker, values) {
			if (typeof values != "undefined" && values.length > 0 && values[0]) {
			    this.value=values[0];
					...
					this.value_text = values[0].frame_number;
					this.searchtext = values[0].frame_number;
          this.$emit("transferInfo", this.value);
			...
			this.$axios({
			  ...
			  console.log(res)
					if (res.data.results.length !== 0) {
						  this.show = true;
							this.slots[0].values = res.data.results.map(function(x) {
								x.text=x.frame_number;
								return x;
							});
					} else {
						this.clear();
