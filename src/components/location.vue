...
<template>
  <div>
    <input type="text" :placeholder='$t("location.b5e34bd0974f11e9a5048c8590a7c397")' @click="popupLocation=true" v-model="area" readonly :disabled="dis_location"/>
    ...
    <div>
      <mt-popup v-model="popupLocation" position="bottom" popup-transition="popup-fade" closeOnClickModal="true" model="true" class="search_pop">
        <mt-picker :slots="dataList" ref="picker2" :visible-item-count="3" :valueKey=this.type :showToolbar="true" @change="onAddressChange">
          <span class="slot_person">{{$t('location.b5e351c0974f11e9a5048c8590a7c397')}}</span>
          <span class="slot_confirm" @click="addressConfirm">{{$t('location.b5e354a4974f11e9a5048c8590a7c397')}}</span>
        </mt-picker>
      </mt-popup>
    </div>
  </div>
